...
```
播报点（C 全程）：
- 开始：`🟢 [C] Issue #N（PROJ-XXXX）开始实装`
- 蓝图派生：`📐 [C] PROJ-XXXX 蓝图 design/changes/PROJ-XXXX.md 完成`
- 实装+自查过：`🔧 [C] PROJ-XXXX 实装完成，构建/自查通过`
- 测试过：`🧪 [C] PROJ-XXXX 测试通过`
- PR 建好：`✅ [C] PROJ-XXXX → PR <url>，Issue 已标「已实装」`
- 失败即停：`⚠️ [C] PROJ-XXXX 在 <Phase> 失败：<原因>，已停，分支保留`
