...
| how | **需求文档** `design/changes/PROJ-XXXX.md` | 逐文件 / 逐函数变更、实装顺序、风险 | 细（够 AI 照着敲代码） | AI（由 Issue 派生） | AI 实装指导；随 PR 顺带 review |
...
4. AI    ① 由 Issue 派生 design/changes/PROJ-XXXX.md（逐文件实装蓝图）
         ② 照蓝图实装
         ③ 测试 case
...
| 项 | 结论 |
|----|------|
| JIRA 读取 | ✅ 已配 `.env`（自 <repo>/.claude/config/claude.env 复制）。修复 `jira_api.py` 的 `load_env` 不去引号 bug 后，`get PROJ-3245` 读通 |
