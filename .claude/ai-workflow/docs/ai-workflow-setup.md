...
# C 实装还需：go 工具链、GOPRIVATE=github.com/MosaviJP/*、能拉私有仓的凭据（SSH key 或 HTTPS token）、docker（ita 测试）
```
...
```bash
ATLASSIAN_USERNAME=  ATLASSIAN_API_KEY=  ATLASSIAN_DOMAIN=mosavi.atlassian.net  JIRA_PROJECT=MOS   # JIRA
SLACK_WEBHOOK_URL=                                                                                  # Slack
GITHUB_WEBHOOK_SECRET=  JIRA_WEBHOOK_TOKEN=  JIRA_TRIGGER_STATUS=待AI处理  APPROVED_LABEL=已审核  PORT=8787  # 接收器
```
- 验证 JIRA：`python3 .claude/ai-workflow/jira_api.py get MOS-3245`；验证 Slack：`bash .claude/ai-workflow/notify.sh 测试`。
...
```jsonc
{
  ...
  "repos": {
    "channel": { "github": "MosaviJP/Mosavi-Channel-Service", "path": "/abs/本机/Mosavi-Channel-Service", "match": [] },
    "group":   { "github": "MosaviJP/Mosavi-Group-Service",   "path": "/abs/本机/Mosavi-Group-Service",   "match": ["group","群组"] }
  }
}
```
...
cd <工具仓>
.claude/ai-workflow/install-into.sh /abs/Mosavi-Channel-Service
cd /abs/Mosavi-Channel-Service        # 干净的默认分支(main)上
...
```bash
gh api repos/MosaviJP/<目标仓>/hooks -X POST -f name=web -F active=true \
...
- 入口：项目 → ⚡ → Create rule，或 `https://<domain>/jira/software/c/projects/MOS/settings/automation`
