...
# ── 5. JIRA 凭据 ────────────────────────────────────────────
step "5. JIRA 凭据"
JIRA_SRC="${JIRA_ENV_SRC:-$HOME/work/<org>/<repo>/.claude/config/claude.env}"
...
if [ -n "$(getenv ATLASSIAN_API_KEY)" ]; then
  if python3 "$JIRA_API" get PROJ-3245 2>/dev/null | grep -q '"key"'; then ok "JIRA 读取验证通过"; else warn "JIRA .env 在但读取失败，检查凭据/网络"; fi
else
  warn "缺 ATLASSIAN_* —— 加到 ${ENV_FILE}：ATLASSIAN_USERNAME/API_KEY/DOMAIN、JIRA_PROJECT=PROJ"
fi
...
cat <<EOF
  JIRA Automation（项目 PROJ → ⚡ → Create rule）：
